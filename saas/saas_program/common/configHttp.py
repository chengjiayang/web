...
class Http():
    ...
    header=None
    host=None
    # def __new__(cls,environment,caseName=None):
    ...
    def __init__(self,environment,caseName=None):
        ...
        if not Http.host:
            Http.host = readConfig.get_host()
            Http.header = readConfig.get_header()
        else:
            if Http.host!=readConfig.get_host():
                Http.host = readConfig.get_host()
                Http.header = readConfig.get_header()
        ...
        # if not Http.header:
        #     Http.header = readConfig.get_header()
        # elif (Http.header) and (Http.header.get("origin") != Http.host):
        #     Http.header = readConfig.get_header()
...
#  可用且文件上传OK，待调
    def execute(self,url,type,params,data):
        ...
        http=requests.request(method=type,url=Http.host+url,params=params,data=data,headers=Http.header,verify=False)

        return http

    def executef(self, url, type, params, data):
        http = requests.request(method=type, url=Http.host + url, params=params, files=data,headers=Http.header,verify=False)
        return http
...
#,headers=Http.header
    ...
    def getauth(self,login_data):
        ...
        Http.lock.acquire()
        try:
           if not Http.header.get("Authorization"):
            ...
            if http.status_code == 200:
                ...
                if http.json()['errCode'] == 0:
                    Http.header["Authorization"] = "bearer {}".format(http.json()['data']["access_token"])
            else:
                self.logger.info("登录失败")
            self.logger.info("登录后header：" + str(Http.header))
        finally:
            Http.lock.release()
    ...
    def newfile(self,name, text):
        '''下载类接口，保存文件'''
        filepath = os.path.join(os.getcwd(),'./saas/saas_program/testFile/DownloadFiles/%s.xlsx'%name)
    ...
    def getResult(self,http,case,result_except):
        ...
        if self.assertInterfacePass(http, case):
            ...
            if type(result_except)==dict:
                if result_except.get("to")=='request_urlparam':
                    result_except.pop('to')
                    for key, value in result_except.items():
                        result_return[key]=re.search(value,http.requst.url).group()
                elif result_except.get("to")=='request_bodyparam':
                    result_except.pop('to')
                    for key, value in result_except.items():
                        result_return = dict(result_return, **{key: jsonpath.jsonpath(eval(http.request.body.decode()), value)[0]})
                            ...
                            if value.startswith('$'):
                                ...
                                try:
                                    result_return = dict(result_return,
                                                         **{key: jsonpath.jsonpath(http.json(), value)[0]})
                                except:
                                    result_return = dict(result_return, **{key: None})
                            # 可处理sql中包含接口取值（sql处理第一步）
                            elif (not value.startswith('$')) and ('$' in value):
                                if ('body$' in value):
                                    pattern1 = u'body\$.[.a-zA-Z_]+'
                                    while re.search(pattern1, value):
                                        value_interface = re.search(pattern1, value).group()[4:]
                                        value = re.sub(pattern1, str(jsonpath.jsonpath(eval(http.request.body.decode()), value_interface)[0]),
                                                       value,
                                                       1)
                                else:
                                    pattern = u'\$.[.a-zA-Z_]+'
                        ...
                        # 可处理mongodb字典中包含的接口取值(mongodb处理第一步)
                            ...
                            try:
                                ...
                                if (not value.startswith('$')) and ('$' in value):
                                    ...
                                    while re.search(pattern, value):
                                        value_interface = re.search(pattern, value).group()
                                        value = re.sub(pattern, str(jsonpath.jsonpath(http.json(), value_interface)[0]),value,1)
                                    try:
                    ...
                    if (["mongodb_type", "mongodb_table", "mongodb_query",
                         "mongodb_order",
                         "mongodb_result"] in list(result_except.keys())) or (
                            ["mongodb_type", "mongodb_table", "mongodb_query",
                             "mongodb_order",
                        ...
                        self.logger.info(result_except["mongodb_query"])
    ...
    def getData_file(self,file):
        ...
        Http.header["Content-Type"] = m.content_type
